...
RSpec.describe 'Arel' do
  ...
  context 'on cast' do
    ...
    it 'provides a cast method' do
      ...
      expect(attribute.cast_with_array('text').to_sql).to be_eql('"a"."sample"::text')
      expect(quoted.cast_with_array('bigint', true).to_sql).to be_eql('ARRAY[1]::bigint[]')
      expect(casted.cast_with_array('string').to_sql).to be_eql("1::string")
    end
  end
end
